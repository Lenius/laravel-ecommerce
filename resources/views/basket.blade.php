@if(Basket::contents())
    <form action="{{route('basket')}}" method="POST">
        ...
        <input type="submit" value="{{ trans('ecommerce::messages.update') }}" class="btn btn-success"> <a href="{{route('basket.destroy')}}" class="btn btn-danger">{{ trans('ecommerce::messages.empty') }}</a>
    </form>
@else
